...
public class UserController {
   ...
   public ResponseEntity<User> updateuser (@PathVariable("id") String id)
   {
	   ...
	   Logger.debug("->->-> calling method UpdateUser for "+id);
	   user = userDAO.get(id);
	   if(user== null)
	   { 
		   Logger.debug("->->->->User does not exist with id "+ id);
	   ...
	   {
	   user.setErrorCode("200");
	   user.setErrorMessage("User updated successfully with id "+id);
	   }
	  /* else
	   {
		   user.setErrorCode("404");
   ...
   public  ResponseEntity<User> userreject(@PathVariable("id") String id) 
   {
